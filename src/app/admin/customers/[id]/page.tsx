...
import { ChevronLeft, Mail, Phone, MapPin, Calendar, MessageSquare, Edit, Trash2, IdCard, Trophy, Target, CalendarDays, FileSignature, DollarSign, Home, Building2, Save, X, Loader2 } from 'lucide-react';
...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [monthStats, setMonthStats] = useState<IndividualCustomerPotentialMonth | null>(null);
    const [allStats, setAllStats] = useState<IndividualCustomerPotentialAll | null>(null);
        ...
        const fetchData = async () => {
            try {
                ...
                const [userData, mStats, aStats] = await Promise.all([
                    ...
                    rankingService.getCustomerMonthlyPotential(id, currentMonth, currentYear),
    ...
    return (
        <div className="max-w-7xl mx-auto pb-10 space-y-6">
            ...
            <div className="space-y-8">
                <div>
                    <h3 className="font-bold text-gray-900 mb-4 text-sm">Current month potential</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <StatItem icon={Trophy} label="Lead Score" value={monthStats?.leadScore || 0} />
                        <StatItem icon={Target} label="Lead position" value={`# ${monthStats?.leadPosition || '-'}`} />
                        <StatItem icon={CalendarDays} label="Viewings requested" value={monthStats?.monthViewingsRequested || 0} />
                        <StatItem icon={CalendarDays} label="Viewings attended" value={monthStats?.monthViewingAttended || 0} />
                        <StatItem icon={FileSignature} label="Contracts signed" value={monthStats?.monthContractsSigned || 0} />
                        <StatItem icon={DollarSign} label="Spending" value={formatCurrency(monthStats?.monthSpending)} />
                        <StatItem icon={Home} label="Properties bought" value={monthStats?.monthPurchases || 0} />
                        <StatItem icon={Building2} label="Properties rented" value={monthStats?.monthRentals || 0} />
                    </div>
                </div>

                <div>
                    <h3 className="font-bold text-gray-900 mb-4 text-sm">All potential</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <StatItem icon={Trophy} label="Lead Score" value={allStats?.leadScore || 0} />
                        <StatItem icon={Target} label="Lead position" value={`# ${allStats?.leadPosition || '-'}`} />
                        <StatItem icon={CalendarDays} label="Viewings requested" value={allStats?.viewingsRequested || 0} />
                        <StatItem icon={CalendarDays} label="Viewings attended" value={allStats?.viewingsAttended || 0} />
                        <StatItem icon={FileSignature} label="Contracts signed" value={allStats?.totalContractsSigned || 0} />
                        <StatItem icon={DollarSign} label="Spending" value={formatCurrency(allStats?.spending)} />
                        <StatItem icon={Home} label="Properties bought" value={allStats?.totalPurchases || 0} />
                        <StatItem icon={Building2} label="Properties rented" value={allStats?.totalRentals || 0} />
                    </div>
                </div>
            </div>
        </div>
