...
export default function OwnersAdvancedSearch({
  ...
  const [contribution, setContribution] = useState({ min: '', max: '' });
  const [value, setValue] = useState({ min: '', max: '' });
  const [properties, setProperties] = useState({ min: '', max: '' });
  const [sales, setSales] = useState({ min: '', max: '' });
  const [rents, setRents] = useState({ min: '', max: '' });
  const [projects, setProjects] = useState({ min: '', max: '' });
  ...
  const handleApply = () => {
    ...
    filters.minContributionPoint = safeParseFloat(contribution.min);
    filters.maxContributionPoint = safeParseFloat(contribution.max);

    filters.minProperties = safeParseInt(properties.min);
    filters.maxProperties = safeParseInt(properties.max);

    filters.minPropertiesForSale = safeParseInt(sales.min);
    filters.maxPropertiesForSale = safeParseInt(sales.max);

    filters.minPropertiesForRents = safeParseInt(rents.min);
    filters.maxPropertiesForRents = safeParseInt(rents.max);

    filters.minProjects = safeParseInt(projects.min);
    filters.maxProjects = safeParseInt(projects.max);
  ...
  const handleResetInternal = () => {
    setTier('All');
    setContribution({ min: '', max: '' });
    setValue({ min: '', max: '' });
    setProperties({ min: '', max: '' });
    setSales({ min: '', max: '' });
    setRents({ min: '', max: '' });
    setProjects({ min: '', max: '' });
    setRanking({ min: '', max: '' });
    ...
    <div className="flex flex-col h-full">
      <div className="space-y-4 flex-1 overflow-y-auto pr-2 pb-4 custom-scrollbar">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <RangeInput label="contribution point" state={contribution} setState={setContribution} />
          <RangeInput label="value" state={value} setState={setValue} placeholder="3.5B" />
          <RangeInput label="properties" state={properties} setState={setProperties} />
          <RangeInput label="for sales" state={sales} setState={setSales} />
          <RangeInput label="for rents" state={rents} setState={setRents} />
          <RangeInput label="projects" state={projects} setState={setProjects} />
