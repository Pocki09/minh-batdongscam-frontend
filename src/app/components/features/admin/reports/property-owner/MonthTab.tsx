...
export default function MonthTab() {
  ...
  // [FIX QUAN TRỌNG]: Sử dụng useMemo để tránh tạo object mới mỗi lần render
  // Table chỉ reload khi tableFilters, month, hoặc year thực sự thay đổi
  ...
  // Đếm filter active (trừ các trường mặc định)
  const activeFiltersCount = Object.keys(tableFilters).filter(k =>
    tableFilters[k as keyof PropertyOwnerFilters] !== undefined && k !== 'name'
  ).length;
  ...
  return (
    <div className="space-y-4 pt-2 animate-in fade-in duration-300">
      ...
      {/* Table - Nhận combinedFilters đã được Memoize */}
      <OwnerContributionTable externalFilters={combinedFilters} />
