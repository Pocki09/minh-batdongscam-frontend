...
import OwnerAdvancedSearch, { LocationSelection } from './OwnerAdvancedSearch';
import { reportService, PropertyOwnerStats } from '@/lib/api/services/statistic-report.service';
...
export default function AllTab() {
    ...
    const [tableFilters, setTableFilters] = useState<PropertyOwnerFilters>({});
    ...
    useEffect(() => {
        const fetchStats = async () => {
            ...
            try {
                const res = await reportService.getPropertyOwnerStats(year);
                setStats(res);
    ...
    const chartData = useMemo(() => {
        ...
        return months.map((monthName, index) => {
            ...
            return {
                name: monthName,
                owners: stats?.totalOwners?.[m] || 0,
                value: stats?.totalContributionValue?.[m] || 0,
                avg: stats?.avgContributionPerOwner?.[m] || 0,
            };
        });
    }, [stats]);
    ...
    const tierData = useMemo(() => {
        ...
        // Calculate total count for each tier across the year (or max)
    ...
    const TIER_COLORS = ['#ec4899', '#eab308', '#9ca3af', '#b45309']; // Pink, Yellow, Gray, Bronze

    // Filter Handlers
    const handleSearch = () => setTableFilters(prev => ({ ...prev, name: keyword }));
    const handleApplyFilters = (newFilters: PropertyOwnerFilters) => {
    ...
    const activeFiltersCount = Object.keys(tableFilters).filter(k => tableFilters[k as keyof PropertyOwnerFilters] !== undefined && k !== 'name').length;
            ...
            <div>
                ...
                {loadingStats ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        {/* Chart 1: Total Owners (Area) */}
                        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                            <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm">
                                <span className="w-3 h-3 rounded-full bg-blue-500 inline-block"></span> Total Active Owners
                            </h3>
                            <div className="h-48 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                                        ...
                                        <Area type="monotone" dataKey="owners" stroke="#3b82f6" strokeWidth={2} fill="url(#colorOwners)" name="Owners" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                        ...
                        {/* Chart 3: Total Contribution Value (Bar) */}
                        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                            <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm">
                                <span className="w-3 h-3 rounded-full bg-emerald-500 inline-block"></span> Total Contribution Value
                            </h3>
                            <div className="h-48 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData} margin={{ top: 5, right: 0, left: -10, bottom: 0 }}>
                                        ...
                                        <Bar dataKey="value" fill="#10b981" radius={[4, 4, 0, 0]} name="Value" barSize={20} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </div>

                        {/* Chart 4: Average Contribution (Line) */}
                        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                            <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm">
                                <span className="w-3 h-3 rounded-full bg-orange-500 inline-block"></span> Avg Contribution per Owner
                            </h3>
                            <div className="h-48 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData} margin={{ top: 5, right: 0, left: -10, bottom: 0 }}>
                                        ...
                                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#9ca3af' }} tickFormatter={(val) => `${val / 1000000}M`} />
                                        <Tooltip content={<CustomTooltip isCurrency />} />
                                        <Area type="monotone" dataKey="avg" stroke="#f97316" strokeWidth={2} fill="url(#colorAvg)" name="Average" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
            ...
            <div className="space-y-4">
                <div className="relative w-full flex gap-2">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text" placeholder="Search Property owner's name"
                ...
                <OwnerContributionTable externalFilters={tableFilters} />
            </div>
