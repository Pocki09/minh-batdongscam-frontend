...
export default function CustomerTable({ externalFilters }: Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await accountService.getAllCustomers(filters);
        setData(res.data);
        ...
        <table className="w-full text-sm text-left text-gray-500">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-4 font-bold text-gray-900">Total Spending</th>
              <th className="px-6 py-4 font-bold text-gray-900">Contracts</th>
              <th className="px-6 py-4 font-bold text-gray-900">Location</th>
              <th className="px-6 py-4 font-bold text-gray-900 text-right">View Detail</th>
            </tr>
          </thead>
            ...
            {data.length === 0 ? (
              ...
              data.map((item, index) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-bold text-gray-900">{item.totalContracts || 0}</td>
