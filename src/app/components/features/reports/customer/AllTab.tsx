    ...
    <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm">
                      <span className="w-3 h-3 rounded-full border border-gray-400 inline-block"></span> Total Customers
                  </h3>
                  <div className="h-40 bg-gray-100/70 rounded-lg flex items-center justify-center text-gray-400 text-sm">
                      [Chart: Line - Growth]
                  </div>
              </div>

              {/* Chart 2: Tier Distribution (Added as requested) */}
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm">
                      <span className="w-3 h-3 rounded-full border border-gray-400 inline-block"></span> Tier Distribution
                  </h3>
                  <div className="h-40 bg-gray-100/70 rounded-lg flex items-center justify-center text-gray-400 text-sm">
                      [Chart: Pie/Donut]
                  </div>
              ...
              {/* Chart 3: Total Spending (Splitted) */}
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm">
                      <span className="w-3 h-3 rounded-full border border-gray-400 inline-block"></span> Total Spending
                  </h3>
                  <div className="h-40 bg-gray-100/70 rounded-lg flex items-center justify-center text-gray-400 text-sm">
                      [Chart: Bar]
                  </div>
              ...
              {/* Chart 4: Average Spending (Splitted) */}
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm">
                      <span className="w-3 h-3 rounded-full border border-gray-400 inline-block"></span> Avg Spending per Customer
                  </h3>
                  <div className="h-40 bg-gray-100/70 rounded-lg flex items-center justify-center text-gray-400 text-sm">
                      [Chart: Line]
                  </div>
      ...
      <div className="space-y-4">
          <div className="relative w-full">
            ...
            <input type="text" placeholder="Customer's name" className="w-full pl-12 pr-24 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 bg-white" />
      ...
      <Modal isOpen={isAdvSearchOpen} onClose={() => { if (!isLocPickerOpen) setIsAdvSearchOpen(false); }} title="Advanced Search">
        <CustomerAdvancedSearch selectedLocations={locations} onRemoveLocation={(loc) => setLocations(prev => prev.filter(l => l !== loc))} onOpenLocationPicker={() => setIsLocPickerOpen(true)} onApply={() => setIsAdvSearchOpen(false)} onReset={() => setLocations([])} />
      </Modal>
